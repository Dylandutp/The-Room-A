...
public static class RoomFeedback
{
    ...
    static void Reset()
    {
        settings = null; clickClip = null; shotClip = null;
    ...
    static void Initialize()
    {
        ...
        for (int i = 0; i < SceneManager.sceneCount; i++)
            BindButtons(SceneManager.GetSceneAt(i));
    }

    static void OnSceneLoaded(Scene scene, LoadSceneMode mode) { BindButtons(scene); }

    static void BindButtons(Scene scene)
    {
        foreach (GameObject root in scene.GetRootGameObjects())
            foreach (Button button in root.GetComponentsInChildren<Button>(true))
                if (button.GetComponent<RoomButtonFeedback>() == null)
                    button.gameObject.AddComponent<RoomButtonFeedback>();
    }
